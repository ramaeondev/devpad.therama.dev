...
<app-link-preview [url]="url" [showInline]="true"> </app-link-preview>
```
...
<img [src]="data.image" loading="lazy" alt="Preview image" />
```
...
it('should display link preview in chat message', () => {
  ...
  const preview = fixture.debugElement.query(By.directive(LinkPreviewComponent));
...
```html
<!-- Add error handling -->
<img [src]="data.image" (error)="onImageError($event)" alt="Preview" />
```
