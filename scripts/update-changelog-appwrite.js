...
if (!DRY_RUN && (!APPWRITE_PROJECT_ID || !APPWRITE_MASTER_API_KEY)) {
  console.error(
    '❌ Missing required environment variables (APPWRITE_PROJECT_ID, APPWRITE_MASTER_API_KEY)'
  )
  process.exit(1)
}
...
function getCommits (fromDateStr, branch) {
  try {
    ...
    if (fromDateStr) {
      ...
      console.log(
        `🔍 Getting commits from ${fromDateStr} to NOW ${branch ? `on branch ${branch}` : ''}`
      )
      ...
      console.log(
        `🔍 Getting commits for YESTERDAY (${yesterdayStr}) ${branch ? `on branch ${branch}` : ''}`
      )
...
async function updateChangelogEntry (databases, dateStr, newMessages) {
  if (DRY_RUN) {
    console.log(
      `[DRY RUN] Would update changelog for ${dateStr} with ${newMessages.length} messages:`
    )
