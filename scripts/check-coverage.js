...
if (!fs.existsSync(summaryPath)) {
  console.error(
    'coverage-summary.json not found. Run the tests with coverage first (npm run test:coverage).'
  )
  process.exit(2)
}
...
const defaultThreshold = process.env.COVERAGE_THRESHOLD
  ? Number(process.env.COVERAGE_THRESHOLD)
  : 80
const thresholds = {
  statements: process.env.COVERAGE_STATEMENTS
    ? Number(process.env.COVERAGE_STATEMENTS)
    : defaultThreshold,
  ...
  branches: process.env.COVERAGE_BRANCHES
    ? Number(process.env.COVERAGE_BRANCHES)
    : defaultThreshold,
  functions: process.env.COVERAGE_FUNCTIONS
    ? Number(process.env.COVERAGE_FUNCTIONS)
    : defaultThreshold
}
