-- Robust fix for activity_logs error
-- Attempt to drop the trigger first to avoid dependency issues
DROP TRIGGER IF EXISTS "on_activity_log_created" ON "public"."activity_logs";

-- Drop the function to ensure we are starting fresh
DROP FUNCTION IF EXISTS "public"."create_notification_for_activity"();

-- Recreate the function with explicit JSONB casting
...
BEGIN
  ...
  -- We cast NEW.device_info to jsonb to ensure ->> operator works even if input is text
