...
serve(async (req) => {
  ...
  const integrationResp = await fetch(
    `${supabaseUrl}/rest/v1/integrations?user_id=eq.${user_id}&provider=eq.google_drive&select=refresh_token`,
    {
      headers: {
        apikey: supabaseKey,
  ...
  await fetch(
    `${supabaseUrl}/rest/v1/integrations?user_id=eq.${user_id}&provider=eq.google_drive`,
    {
      method: 'PATCH',
      headers: {
        apikey: supabaseKey,
