...
serve(async (req) => {
  ...
  if (!client_id || !client_secret) {
    return new Response('Missing Google OAuth env vars', { status: 500, headers: corsHeaders });
  }
  ...
  if (!refresh_token) {
    return new Response('No refresh token found', { status: 400, headers: corsHeaders });
  }
