                  ...
                  <div class="flex items-center gap-3">
                    ...
                    <div class="flex-1">
                      ...
                      <div class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                        <p>{{ device.browser_name || 'Unknown' }} on {{ device.os_name || 'Unknown' }}</p>
                        <p class="mt-0.5">Last seen: {{ formatDate(device.last_seen_at) }}</p>
                        @if (device.country || device.city) {
                          <p class="mt-0.5">Location: {{ device.city }}{{ device.city && device.country ? ', ' : '' }}{{ device.country }}</p>
                        }
                      </div>
                    </div>
                  </div>
...
export class UserDevicesComponent implements OnInit {
    ...
    async saveDeviceName(deviceId: string) {
        ...
        try {
            const success = await this.deviceService.updateDeviceName(deviceId, this.editingDeviceName.trim());
            if (success) {
