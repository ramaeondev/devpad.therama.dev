...
class MockUser {
  async getUserProfile() {
    return { first_name: 'Ann', last_name: 'B' };
  }
}
class MockSupabase {
  auth = {
...
describe('SettingsPanelComponent', () => {
  async function setup(providers: any[] = []) {
    ...
    await TestBed.configureTestingModule({
      imports: [SettingsPanelComponent, RouterTestingModule],
  ...
  it('saveProfile updates profile and handles success and failure', async () => {
    const mockUser: any = {
      ...
      updateUserProfile: jest
        .fn()
        .mockResolvedValue({ first_name: 'Ann2', last_name: 'B2', avatar_url: null }),
    };
  ...
  it('onConfirm2 propagates error when disableUser fails', async () => {
    const disableSpy = jest.fn(async (id: string) => {
  ...
  it('signOut clears auth, signs out supabase, closes panel and navigates', async () => {
    ...
    const mockLoading: any = {
      withLoading: jest.fn(async (fn: any) => {
        await fn();
      }),
    };
