  ...
  template: `
    <div class="fixed inset-0 z-[70] flex items-center justify-center" role="dialog" aria-modal="true">
      ...
      <div class="relative w-full max-w-md rounded-lg bg-white dark:bg-gray-800 shadow-lg border border-gray-200 dark:border-gray-700 p-5">
        ...
        <label class="block text-sm text-gray-600 dark:text-gray-400 mb-1" for="noteName">Note title</label>
        <input
          id="noteName"
        ...
        <div class="mt-4 flex justify-end gap-2">
          ...
          <button type="button" class="px-3 py-1.5 text-sm rounded bg-primary-600 text-white disabled:opacity-50" [disabled]="!canSubmit()" (click)="trySubmit()">Rename</button>
        </div>
      </div>
    </div>
  `,
