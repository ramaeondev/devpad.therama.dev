...
describe('GoogleDriveService', () => {
  ...
  it('saveSelectedFiles updates integration settings and logs activity', async () => {
    ...
    mockSupabase.from = jest.fn().mockReturnValue({ update: jest.fn().mockReturnValue(chain), eq: jest.fn().mockReturnThis() } as any);
  ...
  it('disconnect clears state on success and shows toast', async () => {
    ...
    mockSupabase.from = jest.fn().mockReturnValue({ delete: jest.fn().mockReturnValue(delChain), eq: jest.fn().mockReturnThis() } as any);
  ...
  it('disconnect shows error when delete fails', async () => {
    mockSupabase.from = jest.fn().mockReturnValue({ delete: jest.fn().mockResolvedValue({ error: new Error('no') }), eq: jest.fn().mockReturnThis() } as any);
    service.integration.set({ id: 'i1', access_token: 'at' } as any);
  ...
  it('authFetch throws when no token present', async () => {
    service.integration.set(null);
    await expect((service as any).authFetch('https://example')).rejects.toThrow('Not connected to Google Drive');
  });
  ...
  it('downloadFile returns null and shows toast on non-ok response', async () => {
    // set token
    service.integration.set({ access_token: 'at' } as any);
    jest.spyOn(service as any, 'authFetch').mockResolvedValue({ ok: false, json: jest.fn().mockResolvedValue({ error: 'e' }) } as any);
  ...
  it('deleteFile removes file and returns true on success', async () => {
    ...
    const saveSpy = jest.spyOn(service as any, 'saveSelectedFiles').mockResolvedValue(undefined as any);
  ...
  it('createFolder returns folder and shows success', async () => {
    service.integration.set({ access_token: 'at' } as any);
    const folderData = { id: 'fold1', name: 'N', mimeType: 'application/vnd.google-apps.folder', modifiedTime: 't', parents: [] };
    jest.spyOn(service as any, 'authFetch').mockResolvedValue({ ok: true, json: jest.fn().mockResolvedValue(folderData) } as any);
  ...
  it('renameFile returns true and logs activity on success', async () => {
    service.integration.set({ access_token: 'at' } as any);
    jest.spyOn(service as any, 'authFetch').mockResolvedValue({ ok: true, json: jest.fn().mockResolvedValue({}) } as any);
  ...
  it('pickFiles merges picked files, builds tree and saves selection', async () => {
    ...
    const pickedDocs = [{ id: 'd1', name: 'Doc 1', mimeType: 'text/plain', lastEditedUtc: 't', sizeBytes: 10, url: 'u', iconUrl: 'i', parents: [] }];
    ...
    const mockPickerBuilder = jest.fn().mockImplementation(() => {
      ...
      return {
        ...
        setAppId: function () { return this; },
        setOrigin: function () { return this; },
        setCallback: function (c: any) { cb = c; return this; },
        enableFeature: function () { return this; },
        build: function () { return { setVisible: () => { /* simulate picked */ cb({ action: (global as any).google.picker.Action.PICKED, docs: pickedDocs }); } }; }
      } as any;
    });
  ...
  it('authFetch marks disconnected and shows toast when refresh fails on 401', async () => {
    service.integration.set({ access_token: 'old' } as any);
    (global as any).fetch = jest.fn().mockResolvedValue({ status: 401, ok: false, statusText: 'Unauthorized', json: jest.fn().mockResolvedValue({}) });
  ...
  it('downloadFile returns null when blob retrieval fails', async () => {
    service.integration.set({ access_token: 'at' } as any);
    jest.spyOn(service as any, 'authFetch').mockResolvedValue({ ok: true, blob: jest.fn().mockRejectedValue(new Error('boom')) } as any);
  ...
  it('deleteFile returns false and shows toast on API failure', async () => {
    service.integration.set({ access_token: 'at' } as any);
    jest.spyOn(service as any, 'authFetch').mockResolvedValue({ ok: false, status: 400, statusText: 'Bad', json: jest.fn().mockResolvedValue({}) } as any);
  ...
  it('showPicker handles CANCEL action and shows info toast', async () => {
    ...
    const mockPickerBuilder = jest.fn().mockImplementation(() => {
      ...
      return {
        ...
        setCallback: function (c: any) { cb = c; return this; },
        enableFeature: function () { return this; },
        build: function () { return { setVisible: () => { cb({ action: (global as any).google.picker.Action.CANCEL }); } }; }
      } as any;
    });
  ...
  it('showPicker logs picker errors for unknown action', async () => {
    ...
    const mockPickerBuilder = jest.fn().mockImplementation(() => {
      ...
      return {
        ...
        setCallback: function (c: any) { cb = c; return this; },
        enableFeature: function () { return this; },
        build: function () { return { setVisible: () => { cb({ action: 'UNKNOWN', message: 'oops' }); } }; }
      } as any;
    });
  ...
  it('refreshViaServer success sets new token and shows success toast', async () => {
    mockHttp.post.mockReturnValue({ toPromise: () => Promise.resolve({ access_token: 'nt', expires_at: 12345 }) });
