  ...
  const query: any = {
    ...
    single: jest.fn().mockImplementation(() => {
      ...
      if (lastOp === 'select') {
        ...
        return Promise.resolve({
          data: overrides.selectData ?? overrides.maybeSingleData ?? null,
          error: overrides.selectError ?? null,
        });
      }
      const data =
        lastWriteOp === 'update'
          ? (overrides.updateData ?? null)
          : lastWriteOp === 'insert'
            ? (overrides.insertData ?? null)
            : null;
      const error =
        lastWriteOp === 'update'
          ? (overrides.updateError ?? null)
          : lastWriteOp === 'insert'
  ...
  return {
    ...
    storage: {
      ...
      createSignedUrl: jest.fn().mockResolvedValue({
        data: { signedUrl: overrides.signedUrl || 'https://signed' },
...
describe('NoteService', () => {
  ...
  it('createNote uploads empty content and returns final when no content', async () => {
    ...
    const client = makeMockClient({
      insertData: created,
      updateData: { ...created, content: 'storage://notes/u/n1.md' },
    });
    ...
    const getNoteSpy = jest
      .spyOn(service as any, 'getNote')
      .mockResolvedValue({ ...created, content: 'file content' });
  ...
  it('createNote encrypts when key present and sets is_encrypted', async () => {
    ...
    const client2 = makeMockClient({
      insertData: created,
      updateData: { ...created, content: 'storage://notes/u/n2.md', is_encrypted: true },
    });
  ...
  it('getNote fetches and decrypts content when is_encrypted and has key', async () => {
    ...
    const q: any = {
      select: jest.fn().mockReturnThis(),
      eq: jest.fn().mockReturnThis(),
      single: jest.fn().mockResolvedValue({ data: noteRow, error: null }),
    };
    mockSupabase.from = jest.fn().mockReturnValue(q);
    ...
    (global as any).fetch = jest
      .fn()
      .mockResolvedValue({ ok: true, text: jest.fn().mockResolvedValue('enc:payload') });
  ...
  it('verifyUpload succeeds when signed url and fetch return content', async () => {
    ...
    (global as any).fetch = jest
      .fn()
      .mockResolvedValue({ ok: true, text: jest.fn().mockResolvedValue('file content') });
  ...
  it('verifyUpload throws when fetch returns non-ok', async () => {
    ...
    await expect((service as any).verifyUpload('u/n1.md')).rejects.toThrow(
  ...
  it('updateNote migrates raw DB content to storage and marks encrypted when encryption enabled', async () => {
    ...
    const selectQ: any = {
      ...
      single: jest
        .fn()
        .mockResolvedValue({ data: { id: 'n3', content: 'raw old content' }, error: null }),
      update: jest.fn().mockImplementation(() => ({
        eq: jest.fn().mockReturnThis(),
        select: jest.fn().mockReturnThis(),
        single: jest.fn().mockResolvedValue({
          data: { id: 'n3', content: 'storage://notes/u/n3.md', is_encrypted: true },
          error: null,
        }),
      })),
    };
    ...
    (global as any).fetch = jest
      .fn()
      .mockResolvedValue({ ok: true, text: jest.fn().mockResolvedValue('ok') });
  ...
  it('updateNote overwrites storage when dto.content provided and encrypts when needed', async () => {
    ...
    const selectQ: any = {
      ...
      single: jest
        .fn()
        .mockResolvedValue({ data: { id: 'n4', content: 'storage://notes/u/n4.md' }, error: null }),
      update: jest.fn().mockImplementation(() => ({
        eq: jest.fn().mockReturnThis(),
        select: jest.fn().mockReturnThis(),
        single: jest.fn().mockResolvedValue({
          data: { id: 'n4', content: 'storage://notes/u/n4.md', is_encrypted: true },
          error: null,
        }),
      })),
    };
  ...
  it('uploadDocument rejects large files and blocked extensions', async () => {
    ...
    await expect(service.uploadDocument('u', bigFile, null)).rejects.toThrow(
      'File size exceeds 5MB limit',
    );

    const exeFile = new File([new ArrayBuffer(100)], 'bad.exe', {
      type: 'application/octet-stream',
    });
    await expect(service.uploadDocument('u', exeFile, null)).rejects.toThrow(
  ...
  it('decryptNoteAtSource throws when note not found', async () => {
    ...
    mockSupabase.from = jest.fn().mockReturnValue(q);
    await expect(service.decryptNoteAtSource('n1', 'u')).rejects.toThrow(
  ...
  it('decryptNoteAtSource re-uploads and marks unencrypted when content already plain', async () => {
    ...
    const q: any = {
      select: jest.fn().mockReturnThis(),
      eq: jest.fn().mockReturnThis(),
      single: jest.fn().mockResolvedValue({ data: note, error: null }),
    };
    mockSupabase.from = jest.fn().mockReturnValue(q);
    mockSupabase.storage = {
      from: jest.fn().mockReturnThis(),
  ...
  it('decryptNoteAtSource throws when encrypted and key not loaded', async () => {
    ...
    await expect(service.decryptNoteAtSource('n3', 'u')).rejects.toThrow(
  ...
  it('decryptNoteAtSource decrypts and reuploads when key present', async () => {
    ...
    const storage = {
      from: jest.fn().mockReturnThis(),
      createSignedUrl: jest.fn().mockResolvedValue(signedResp),
      upload: jest.fn().mockResolvedValue({ error: null }),
    } as any;
    mockSupabase.from = jest.fn().mockReturnValue(q);
    ...
    (global as any).fetch = jest
      .fn()
      .mockResolvedValue({ ok: true, text: jest.fn().mockResolvedValue('encpayload') });
  ...
  it('fetchStorageContent returns decrypted content when key present', async () => {
    ...
    (global as any).fetch = jest
      .fn()
      .mockResolvedValue({ ok: true, text: jest.fn().mockResolvedValue('encdata') });
    mockEncryption.hasKey.mockReturnValue(true);
  ...
  it('fetchStorageContent throws when signed URL creation fails', async () => {
    mockSupabase.storage.createSignedUrl.mockResolvedValue({ data: null, error: { code: 'ERR' } });
    await expect(
      (service as any).fetchStorageContent('storage://notes/u/n1.md'),
    ).rejects.toBeTruthy();
  });
  ...
  it('fetchStorageContent throws when fetch returns non-ok', async () => {
    ...
    await expect((service as any).fetchStorageContent('storage://notes/u/n1.md')).rejects.toThrow(
  ...
  it('getFileBlob returns decrypted blob when encrypted and key present', async () => {
    ...
    jest
      .spyOn(service as any, 'getNote')
      .mockResolvedValue({ content: 'storage://notes/u/nfile.md', is_encrypted: true } as any);
    mockSupabase.storage.createSignedUrl.mockResolvedValue({
      data: { signedUrl: 'https://signed' },
      error: null,
    });
  ...
  it('createNote throws when storage upload fails', async () => {
    ...
    await expect(
      service.createNote('u', { title: 'T', content: 'ok' } as any),
    ).rejects.toBeTruthy();
  });
  ...
  it('createNote falls back to plaintext when encryption fails', async () => {
    ...
    const client = makeMockClient({
      insertData: created,
      updateData: { ...created, content: 'storage://notes/u/n5.md' },
    });
    client.storage.upload = jest.fn().mockResolvedValue({ error: null });
    client.storage.createSignedUrl = jest
      .fn()
      .mockResolvedValue({ data: { signedUrl: 'https://signed' }, error: null });
    mockSupabase.from = client.from;
    mockSupabase.storage = client.storage;
  ...
  it('updateNote throws when storage upload fails during migration', async () => {
    const selectQ: any = {
      ...
      single: jest
        .fn()
        .mockResolvedValue({ data: { id: 'nm', content: 'raw old content' }, error: null }),
      update: jest.fn().mockImplementation(() => ({
        eq: jest.fn().mockReturnThis(),
        select: jest.fn().mockReturnThis(),
        single: jest.fn().mockResolvedValue({ data: { id: 'nm' }, error: null }),
      })),
    };
    mockSupabase.from = jest.fn().mockReturnValue(selectQ);
    mockSupabase.storage = {
  ...
  it('decryptNoteAtSource throws when signed URL creation fails', async () => {
    ...
    mockSupabase.storage = {
      from: jest.fn().mockReturnThis(),
      createSignedUrl: jest.fn().mockResolvedValue({ data: null, error: { code: 'ERR' } }),
    } as any;

    await expect(service.decryptNoteAtSource('nd1', 'u')).rejects.toThrow(
      'Failed to fetch encrypted content from storage',
    );
  });
  ...
  it('decryptNoteAtSource throws when download fails', async () => {
    ...
    await expect(service.decryptNoteAtSource('nd2', 'u')).rejects.toThrow(
      'Failed to download encrypted content',
    );
  });
  ...
  it('decryptNoteAtSource throws when re-upload fails for plain content', async () => {
    ...
    mockSupabase.storage = {
      from: jest.fn().mockReturnThis(),
      upload: jest.fn().mockResolvedValue({ error: new Error('reupload fail') }),
    } as any;

    await expect(service.decryptNoteAtSource('nd3', 'u')).rejects.toThrow(
      'Failed to re-upload content',
    );
  });
  ...
  it('uploadDocument returns updated note on success', async () => {
    ...
    const client = makeMockClient({
      insertData: created,
      updateData: { ...created, content: 'storage://notes/u/ud1.md' },
    });
    client.storage.upload = jest.fn().mockResolvedValue({ error: null });
    client.storage.createSignedUrl = jest
      .fn()
  ...
  it('deleteNote creates notification when activity exists', async () => {
    const qSelect: any = {
      select: jest.fn().mockReturnThis(),
      eq: jest.fn().mockReturnThis(),
      single: jest.fn().mockResolvedValue({ data: { title: 'T' }, error: null }),
    };
    const qDelete: any = {
