...
describe('SigninComponent', () => {
  it('onSubmit success sets auth and navigates', async () => {
    await TestBed.configureTestingModule({ imports: [SigninComponent, (await import('@angular/router/testing')).RouterTestingModule], providers: [ { provide: (await import('../../../../core/services/supabase.service')).SupabaseService, useClass: MockSupabase }, { provide: (await import('../../../../core/services/toast.service')).ToastService, useClass: MockToast }, { provide: (await import('../../../../core/services/auth-state.service')).AuthStateService, useClass: MockAuthState }, { provide: (await import('../../../../core/services/device-fingerprint.service')).DeviceFingerprintService, useClass: MockDevice }, { provide: (await import('../../../../core/services/activity-log.service')).ActivityLogService, useClass: MockActivity } ] }).compileComponents();
  ...
  it('signInWithGoogle handles error paths gracefully', async () => {
    const sup = new MockSupabase(); sup.authDirect.signInWithOAuth = jest.fn().mockRejectedValue(new Error('bad'));
    await TestBed.configureTestingModule({ imports: [SigninComponent, (await import('@angular/router/testing')).RouterTestingModule], providers: [ { provide: (await import('../../../../core/services/supabase.service')).SupabaseService, useValue: sup }, { provide: (await import('../../../../core/services/toast.service')).ToastService, useClass: MockToast } ] }).compileComponents();
