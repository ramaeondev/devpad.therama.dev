    ...
    <div class="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900">
      <div class="text-center">
        @if (loading()) {
          ...
          ></div>
          <p class="mt-4 text-gray-600 dark:text-gray-400">Completing Google sign in...</p>
        }
        @if (error()) {
...
export class GoogleCallbackComponent implements OnInit {
  ...
  async ngOnInit() {
    try {
      ...
      this.error.set(err.message || 'Failed to complete Google sign in');
      this.toast.error('Failed to sign in with Google');
    } finally {
